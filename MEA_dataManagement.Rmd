...
findSWPvalues <- function(time, trace, start.frame){ 
  ...
  # plot(trace, type = "l") + abline(v=start.frame)
                        ...
                        ### Initial baseline
  bl = mean(trace[ 1:(start.frame-2)]) # average from 0 to before stimFrame
...
# to find half max frame, substract the halfMax V from the trace and pick the closest to 0.
  ...
  #  plot(hyperpolTrace) + abline(v=max.slope.frame) 
...
"/Users/j/Documents/GitHub/MCS-data_analysis/MCS_dataManager.py"
input   - All the ".h5" files in the home folder of this project.
output  - a "Python-analysis" folder with ">savename<.csv" files with units s and mV.
...
for (f in files){
  ...
  for (i in 2:(length(names(traces.all))-1) ) {
...
for (f in files){
  ...
  for (c in 2:33 ){
    ...
    from = which(trace[(to-(90*samplerate)):to] == max(trace[(to-(90*samplerate)):to]) ) + to-(105*samplerate) # start.frame = from
    ...
    abline(v = traces.all$time[SWPvalues$min.slope.frame], col="red") +
    abline(v = traces.all$time[SWPvalues$halfMaxFrame], col="green", lty = 2) +
    abline(v = traces.all$time[SWPvalues$max.slope.frame], col="red") +
    abline(v = traces.all$time[SWPvalues$depolFrame], col="red", lty = 2) +
    abline(v = traces.all$time[SWPvalues$halfRepolframe], col="green") +
...
# setwd(batchfolder)
# traces.all = read.csv(f)
# traces.all$C_8 = traces.all$C_10
...
output  - ">parameter<.pdf" plots of the heatmaps of all the parameters. "spatial.correlation.csv" file with the spearman correlation test, to both MEA dimensions, for all parameters
```{r}
setwd(batchfolder)
...
input - The "array_SWPparameters.csv" files from 2. Put the folders in manually.
...
for (g in genotypes){
  ...
  for (f in folders[1:length(folders)]){
    ...
    speed.insumo = data.frame(row= array_SWPparameters$row ,
                             ...
                             min.slope.frame = (array_SWPparameters$min.slope.frame/samplerate),
                             halfMaxframe = (array_SWPparameters$halfMaxFrame /samplerate),
                             depDur = (array_SWPparameters$depDur) ) %>% na.omit()
    
    # calculate slope by regression and by 2 points
    for (p in which(names(speed.insumo)%in%c("min.slope.frame","halfMaxframe" )) ){
    ...
    duration = mean(speed.insumo$depDur)
    SEM = sd(speed.insumo$depDur)/length(speed.insumo$depDur)
    duration.correlation = cor.test(speed.insumo$location,speed.insumo$depDur)$p.value %>% round(5)
    duration.rho = cor.test(speed.insumo$location,speed.insumo$depDur)$estimate %>% round(5)
  ...
  data.stats = rbind(data.stats, 
                    data.frame("genotype" = g,
                              ...
                              "slope.reg.mean" = mean( speeds$speed.regression[which(speeds$parameter=="min.slope.frame")] ),
                              "slope.reg.SEM" = sd(speeds$speed.regression[which(speeds$parameter=="min.slope.frame")])/
                                length(speeds$speed.regression[which(speeds$parameter=="min.slope.frame")]),
                              "slope.2e.mean" = mean( speeds$speed.2point[which(speeds$parameter=="min.slope.frame")] ),
                              "slope.2e.SEM" = sd( speeds$speed.2point[which(speeds$parameter=="min.slope.frame")] )/
                                length(speeds$speed.2point[which(speeds$parameter=="min.slope.frame")]),
                              "halfMax.reg.mean" = mean( speeds$speed.regression[which(speeds$parameter=="halfMaxframe")] ),
                              "halfMax.reg.SEM" = sd(speeds$speed.regression[which(speeds$parameter=="halfMaxframe")])/
                                length(speeds$speed.regression[which(speeds$parameter=="halfMaxframe")]),
                              "halfMax.2e.mean" = mean( speeds$speed.2point[which(speeds$parameter=="halfMaxframe")] ),
                              "halfMax.2e.SEM" = sd( speeds$speed.2point[which(speeds$parameter=="halfMaxframe")] )/
                                length(speeds$speed.2point[which(speeds$parameter=="halfMaxframe")]))
                        )
...
#mean( speeds$speed.regression[which(speeds$parameter=="min.slope.frame")] )
```
...
input - IN THE HOME FOLDER! >genotype< folder with the "array_SWPparameters.csv" file of each plant
...
```{r}
setwd(home)
genotypes = c("col0", "msl10-1" , "slah3-3") #,,"slah3-3", "col0","msl10-1" "slah3-1",
for (g in genotypes){
  folders = list.dirs()[grep(g, list.dirs())]
  ...
  for (f in folders[1:length(folders)]){
    ...
    # HERE USE normalized or data as is?
    array_SWPparameters = read.csv( "normalized_SWPparameters.csv" ) # "normalized_SWPparameters.csv" "array_SWPparameters.csv"
    ###
    ...
    for(e in 1:32){
      ...
      if ( !unique(array_SWPparameters$electrode)[e] %in% names(genotype_params) ){
        # got it!! it is saving in which ever order and then changing the name. I assumed it'd always open the electrodes in the same order, but not!. now, how allocate data to a part of the list independently?
    ...
    setwd(home)
    save(genotype_params, file = paste(g, "_CompiledParameters.RData",  sep="") )
  }
}
```
...
input - The >genotype<_CompiledParameters.RData files in the home folder.
output - heatmaps for the mean  of each parameter for every electrode and every genotype.Also a >genotype<_mean.spatial.correlation.csv file for every genotype. The summary can be changed to the median if desired.
```{r}
setwd(home)
files = list.files()[grep("RData", list.files())]
for (f in files) {
  load(f)
  genotype = genotype_params$C_11$plant[1] %>% substr(20,nchar(genotype_params$C_11$plant[1]))
  genotype.summary = NULL
  
  
  # sacar promedios/medianas de todo, con SEM/?. columna a columna y guardar en dataframe?
...
# 7. Generate scatter plots and linear model for those variables that have significant correlation in point 6.
```{r}
list.files()
load("slah3-1_CompiledParameters.RData"  )
# Scatter plot the parameters.
tst = data.frame()

for(e in 1:length(genotype_params) ){
  tst = rbind( tst, data.frame( "value"= genotype_params[[e]]$repol.amplitude, 
                           "electrode" = names(genotype_params)[e], 
                           "parameter"= "halfMaxFrame",
                           "plant"    = genotype_params[[e]]$plant)
              )
}
tst = separate(tst, 2, c("row","column"),sep="_")
tst$column = as.numeric(tst$column)
# make linear model
model <- lm( tst$value ~ tst$column )
model.slope = coef(model)[[2]] # cm/s
plot(tst$column, tst$value) +
  abline(model, lwd=3)
```
...
data.compilation
write.csv(data.compilation, "2312xx.speeds.csv",row.names = FALSE)
```
...
# remove electrode
traces.all = traces.all[,-c(which(names(traces.all)=="C_1"))]
    # downsample for plotting.
trace.plot = NULL
...
trace.plot = data.frame(trace.plot)
names(trace.plot) = c("time", "C_11","C_10","C_09","C_08","C_07","B_11","B_10","B_09","B_08","B_07","A_11","A_10" ,"A_09", "A_08","A_07", "A_06","C_06","A_05", "A_04","A_03","A_02","A_01","B_05","B_04","B_03","B_02","B_01","C_05","C_04","C_03","C_02","genotype")

trace.plot = gather(trace.plot, "e", "mV", 2:(ncol(trace.plot)-1) )

trace.plot = separate(trace.plot, 3, c("row", "column"),sep = "_" )
trace.plot$column = as.character(trace.plot$column)
...
ggplot( trace.plot, aes(time, mV, col=column) ) +
  geom_line() +
  scale_color_manual(values = rainbow(length(unique(trace.plot$column)),start=0,end=0.5 ))+ #'orange', 'pink', 'red','black',
  #                           'blue','green','purple','turquoise',
  #                           'yellow','salmon','darkgreen' ))
  #scale_color_steps(low = "#C1666B", high = "#ADE25D")  +
...
ggsave("2023-12-14T16-18-16col0.all-line.pdf")
```
